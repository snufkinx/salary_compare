...
class CSVOutput:
    ...
    def render_single(self, result: TaxResult, output_file: Optional[str] = None):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerow(
                ["Deduction Type", "Amount", "Rate (%)", "Description", "Calculation Details"]
            )
            ...
            if result.income_tax_brackets:
                ...
                writer.writerow(
                    ["Lower Bound", "Upper Bound", "Rate (%)", "Taxable Amount", "Tax Amount"]
                )
    ...
    def render_comparison(self, results: List[TaxResult], output_file: Optional[str] = None):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for result in results:
                net_percentage = (
                    (result.net_salary / result.gross_salary) * 100
            ...
            for i, result in enumerate(results):
                writer.writerow([])  # Empty row
                writer.writerow(
                    [f"Detailed Breakdown {i+1}: {result.country} {result.employment_type}"]
                )
                writer.writerow(
                    ["Deduction Type", "Amount", "Rate (%)", "Description", "Calculation Details"]
                )
