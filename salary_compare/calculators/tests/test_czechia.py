...
class TestSalariedEmployeeCzechia:
    ...
    def test_50000_euro_salary(self):
        ...
        assert (
            result.tax_base == result.gross_salary
        ), "Tax base should equal gross salary for salaried employees"
        ...
        # Verify rates
        assert abs(social_security - Decimal("50000") * Decimal("0.065")) < Decimal(
            "1"
        ), "Social security should be 6.5%"
        assert abs(health_insurance - Decimal("50000") * Decimal("0.045")) < Decimal(
            "1"
    ...
    def test_tax_threshold_calculation(self):
        ...
        assert (
            len(result.income_tax_brackets) == 1
        ), "Should have only one tax bracket below threshold"
    ...
    def test_social_security_and_health_rates(self):
        ...
        assert abs(social_security - expected_social) < Decimal(
            "1"
        ), "Social security should be exactly 6.5%"
        assert abs(health_insurance - expected_health) < Decimal(
            "1"
