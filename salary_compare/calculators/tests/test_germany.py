...
class TestSalariedEmployeeGermany:
    ...
    def test_50000_euro_salary(self):
        ...
        assert result.tax_base > Decimal('35000'), "Tax base should be reasonable after social security deductions"
    ...
    def test_100000_euro_salary(self):
        ...
        social_security_total = sum(
            deduction.amount for deduction in result.deductions 
            if 'Insurance' in deduction.name
        )
    ...
    def test_tax_free_allowance(self):
        ...
        assert social_security_total > Decimal('0'), "Should still have social security contributions"
    ...
    def test_progressive_tax_brackets(self):
        ...
        assert abs(total_bracket_tax - income_tax) < Decimal('1'), \
    ...
    def test_solidarity_surcharge(self):
        ...
        calculator = SalariedEmployeeGermany(Decimal('80000'))  # High enough to trigger solidarity surcharge
        result = calculator.calculate_net_salary()
        
        solidarity_surcharge = next(
            (d.amount for d in result.deductions if d.name == "Solidarity Surcharge"), 
            Decimal('0')
        )
        
        # Should have solidarity surcharge for high income
        assert solidarity_surcharge > Decimal('0'), "Should have solidarity surcharge for high income"
        
        # Verify rate (5.5% of income tax)
        income_tax = next((d.amount for d in result.deductions if d.name == "Income Tax"), Decimal('0'))
        expected_solidarity = income_tax * Decimal('0.055')
        assert abs(solidarity_surcharge - expected_solidarity) < Decimal('1'), \
    ...
    def test_social_security_rates(self):
        ...
        pension_insurance = next((d.amount for d in result.deductions if d.name == "Pension Insurance"), Decimal('0'))
        health_insurance = next((d.amount for d in result.deductions if d.name == "Health Insurance"), Decimal('0'))
        unemployment_insurance = next((d.amount for d in result.deductions if d.name == "Unemployment Insurance"), Decimal('0'))
        long_term_care_insurance = next((d.amount for d in result.deductions if d.name == "Long_Term_Care Insurance"), Decimal('0'))
        
        # Verify rates with ceilings applied
        # Pension: 9.3% of €96,000 (ceiling) = €8,928
        assert abs(pension_insurance - Decimal('8928')) < Decimal('1'), \
            f"Pension insurance should be €8,928 (capped), got {pension_insurance}"
        # Health: 9.1% of €62,100 (ceiling) = €5,651.10
        assert abs(health_insurance - Decimal('5651.10')) < Decimal('1'), \
            f"Health insurance should be €5,651.10 (capped), got {health_insurance}"
        # Unemployment: 1.3% of €96,000 (ceiling) = €1,248
        assert abs(unemployment_insurance - Decimal('1248')) < Decimal('1'), \
            f"Unemployment insurance should be €1,248 (capped), got {unemployment_insurance}"
        # Long-term care: 2.0% of €62,100 (ceiling) = €1,242
        assert abs(long_term_care_insurance - Decimal('1242')) < Decimal('1'), \
    ...
    def test_edge_case_low_salary(self):
        ...
        assert social_security_total > Decimal('0'), "Should have social security even for low salary"
    ...
    def test_edge_case_very_high_salary(self):
        ...
        assert highest_bracket.rate == Decimal('0.45'), "Should use highest tax bracket for very high income"
        ...
        assert solidarity_surcharge > Decimal('0'), "Should have solidarity surcharge for very high income"
