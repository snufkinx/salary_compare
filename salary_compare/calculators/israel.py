...
class SalariedEmployeeIsrael(SalariedEmployee):
    ...
    def __init__(self, gross_salary: Decimal):
        ...
        # Initialize currency converter (ILS to EUR)
        self.currency_converter = get_currency_converter(from_currency="ILS", to_currency="EUR")
        ...
        # Convert tax brackets from ILS to EUR
        self.tax_brackets_eur = []
        for lower, upper, rate in self.tax_brackets_ils:
            lower_eur = self.currency_converter.convert(Decimal(str(lower)))
            if upper == float("inf"):
                upper_eur = Decimal("inf")
            else:
                upper_eur = self.currency_converter.convert(Decimal(str(upper)))
            self.tax_brackets_eur.append((lower_eur, upper_eur, Decimal(str(rate))))
        ...
        self.keren_hishtalmut_cap = self.currency_converter.convert(Decimal("188544"))
    ...
    def calculate_net_salary(self) -> TaxResult:
        ...
        self._calculate_progressive_tax(taxable_income, result)
